...
int update_val(char **env, char *var_name)
{
	...
	while (env[i])
	{
		if (!ft_strncmp(extract_var_name(env[i]),
			...
			ft_strlen(extract_var_name(var_name)) - ft_strlen(extract_var_name(env[i])) == 1 && var_name[ft_strlen(extract_var_name(var_name)) - 1] == '+')
		{
...
int check_env_var(char *env_var)
{
	...
	if (!ft_strcmp(extracted_var, "_"))
		return (-1);
	else if (check_dash_err(extracted_var) || !is_validated(extracted_var, env_var))
		return (1);
	return (0);
}
...
int ft_export(t_data *data)
{
	...
	if (!ft_strcmp(data->spllited_cmd_buf[0], "export") && !data->spllited_cmd_buf[1])
	{
		display_export(data);
		return (0);
	}
