...
/*   Updated: 2022/07/14 16:20:03 by ykhadiri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_command *ft_parse(t_token *tokens, t_env *lenv)
{
	...
	size = 0;
	while (current_token && current_token->type != N_line && current_token->type != PIPE)
	{
		if ((current_token->type == HERDOC || current_token->type == APPEND || current_token->type == REDIN || current_token->type == REDOUT) && current_token->next->type == WORD)
		{
			current_token = current_token->next->next;
		}

		if (current_token && (current_token->type == WORD || current_token->type == DBQUOTE || current_token->type == QUOTE))
		{
			size++;
	...
	while (first_word)
	{
		...
		{
			if (first_word->type == HERDOC || first_word->type == APPEND || first_word->type == REDIN || first_word->type == REDOUT)
			{
				// push redirections
			...
			else if (first_word && (first_word->type == WORD || first_word->type == DBQUOTE || first_word->type == QUOTE))
			{
		...
		if (first_word->type == PIPE || first_word->type == N_line)
			fill_command(&cmd, initialize_command(cmd_args, redirections, first_word));
		first_word = first_word->next;
	}
